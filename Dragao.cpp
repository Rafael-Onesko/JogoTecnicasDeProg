...
namespace Entidades {
	namespace Personagens {
		...
		Dragao::Dragao(float pX, float pY, Listas::ListaEntidades* listEntid, Jogador* jog1, Jogador* jog2) : Inimigo(100.f, 100.f, 80.f, dragao, jog1, jog2) {
			this->setPosX(pX);
			this->setPosY(pY);
			if (listEntid)
				this->lE = listEntid;
		...
		void Dragao::tacarFogo() {
			...
			bolaDeFogo = new Bola_De_Fogo(posX - 10.f, posY + 50.f, direcao);
